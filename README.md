# Kiss — Figma Plugin

Набор утилит для помощи в работе: выравнивание секций, управление приоритетами, поиск похожих объектов и другие инструменты работы в Figma.
Плагин поддерживает компактную плавающую панель, через которую можно запускать любую команду в один клик — без необходимости каждый раз открывать меню Plugins

<img width="2374" height="1624" alt="Frame 10698003" src="https://github.com/user-attachments/assets/80fc2a25-7b48-4d4d-96ed-9ab61b97d6c3" />

## Функционал

### ⚡ Выровнять все секции
Автоматически выравнивает все секции на странице по сетке — расставляет их в ряды с одинаковыми отступами. Ничего не нужно выделять, плагин берёт все секции сам.

### ➡️ Увеличить секцию
Увеличивает ширину выделенной секции, не смещая левую границу. Все секции, расположенные правее, автоматически сдвигаются — структура макета сохраняется.

### 📐 Выровнять + тёмная
Выравнивает фреймы внутри секции. Автоматически дублирует их во второй ряд снизу и переключает дубли в тёмную тему через переменные Figma (Appearance → Dark). Не нужно вручную копировать экраны и менять тему.

### ➕ Дублировать
Дублирует выбранный фрейм прямо внутри секции: клон ставится сразу после оригинала, а все секции того же ряда, стоящие правее, сдвигаются на нужное расстояние. Удобно когда нужно быстро добавить ещё один экран в уже выстроенный ряд — не нужно вручную двигать соседние секции.

### 🗂️ Обернуть + выровнять
Выдели любые объекты — плагин обернёт их в новую секцию и выровняет в один ряд. Если выделена уже существующая секция — просто выровняет содержимое внутри неё. Работает с любыми типами объектов: фреймы, компоненты, группы и т.д.

### 🔄 Замена instance
Выдели несколько объектов, последним — эталонный экземпляр. Плагин заменит все остальные выделенные объекты на копии эталона, сохранив их позиции.

### ✂️ Нарезка 1px
Нарезает выделенные объекты на слайсы по 1 px — удобно для подготовки ассетов.

### 🖼️ Размеры в арт
Проставляет размеры текстовых объектов под соответствующими артбордами. Каждому объекту должно соответствовать два текстовых слоя (ширина и высота).

### 🔍 Найти похожие
Выдели любой объект — фрейм, компонент, прямоугольник, что угодно. Плагин найдёт по всей странице объекты с таким же именем и такими же размерами и выделит их все, включая исходный.

### 🟠 Средний приоритет
Добавляет к выделенному объекту тег «Средний приоритет» (оранжевый). Если тег уже есть — обновляет его.

### 🔴 Высокий приоритет
Добавляет тег «Высокий приоритет» (красный). Если тег уже есть — обновляет его.

### 🟢 Задача выполнена
Добавляет тег «Задача выполнена» (зелёный). Если тег уже есть — обновляет его.

### 🎛️ Панель инструментов
Открывает компактную плавающую панель со всеми командами плагина в виде кнопок. Запускаешь один раз — панель висит поверх канваса, можно перетащить в удобное место. По умолчанию свёрнута (4 кнопки), стрелка **›** раскрывает все команды. После нажатия любой кнопки панель остаётся открытой.

### 📖 FAQ
Открывает встроенную справку по всем командам плагина прямо внутри Figma.

<img width="2274" height="1608" alt="Frame 10698005" src="https://github.com/user-attachments/assets/701b010b-78a9-4b29-b4a3-e42c0aaeecba" />

## Особенности

- Не требует API-ключей и внешних сервисов — всё работает локально внутри Figma
- Работает с любыми типами объектов: фреймы, компоненты, главные компоненты, группы
- Поиск по всей странице, включая содержимое секций
- Переключение тёмной темы через переменные Figma (Variable Modes / Appearance)
- Компактная плавающая панель инструментов для быстрого доступа ко всем командам
- Встроенный FAQ с описанием каждой команды

## Как установить

1. Скачай или склонируй репозиторий
2. Открой Figma → Plugins → Development → Import plugin from manifest
3. Выбери `manifest.json` из папки проекта
4. Плагин появится в списке плагинов разработки
